...
### Sécurité fail-closed (§3.2 audit v0.2.1)
...
|------------|-------------|------------|
| `create`   | Crée un nouveau token (affiché une seule fois) | client_name, permissions, tool_ids (["all"] accepté), expires_days, email |
| `list`     | Liste tous les tokens (sans valeur brute) | — |
| `info`     | Détails d'un token | client_name |
| `update`   | Modifie permissions/tool_ids/email sans révoquer | client_name, permissions?, tool_ids? (["all"] accepté), email? |
| `revoke`   | Supprime un token | client_name |
