...
> **Version** : 0.1.2 | **Date** : 2026-03-05 | **Auteur** : Cloud Temple
> **Projet** : mcp-tools | **Licence** : Apache 2.0
> **Statut** : 🚧 Implémentation en cours — 6/28 tools validés (shell, network, http, perplexity_search, system_health, system_about)
...
| Contrainte        | shell                | network                      | http                      |
| ----------------- | -------------------- | ---------------------------- | ------------------------- |
| `--network`       | `none`               | `bridge`                     | `bridge`                  |
| `--cap-add`       | aucun                | `NET_RAW`                    | aucun                     |
| Anti-SSRF         | N/A (pas de réseau)  | RFC 1918 (IP directe)        | RFC 1918 + résolution DNS |
| Exécutable        | bash/sh/python3/node | ping/dig/nslookup/traceroute | curl                      |
| Nommage conteneur | `sandbox-{uuid}`     | `network-{uuid}`             | `http-{uuid}`             |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------- | ------ | ------- |
| **Phase 1** | ssh, shell, http, network, docker, files, date, calc, generate, mcp_call, perplexity (×3)                                       | 13     | 2-3 sem |
