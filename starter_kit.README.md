...
- Expose des **outils** (`@mcp.tool()`) via Streamable HTTP
...
| Communication    | `httpx` + `httpx-sse`   | Client Streamable HTTP vers le serveur |
...
│       ├── client.py           # Client Streamable HTTP vers le serveur
...
#### Middleware ASGI — Ligne de défense principale

Le middleware `AuthMiddleware` (dans `auth/middleware.py`) est la **première
ligne de défense**. Il **DOIT bloquer** toute requête sur une route protégée
si aucun token Bearer valide n'est fourni :

```python
# ⚠️ CRITIQUE : ce bloc DOIT exister dans AuthMiddleware.__call__()
# Sans lui, /mcp et /api/* sont accessibles sans authentification !
...
- Routes publiques (`/health`, `/healthz`, `/ready`, `/favicon.ico`, `/static/*`) → passent sans token
- Toutes les autres routes (`/mcp`, `/api/*`, `/`) → **HTTP 401** `{"error": "Authorization header required"}` sans token valide

> ⚠️ **Bug historique** : le starter-kit v0 laissait passer les requêtes sans token
> (il injectait `token_info=None` dans le contextvar sans bloquer).
> Cela signifiait que `/mcp` retournait une erreur JSON-RPC au lieu de `401`.
> **Toujours vérifier** que le middleware bloque AVANT de passer au handler.

#### Pattern d'auth dans les outils MCP
...
> **Double sécurité** : le middleware bloque au niveau HTTP (401), et
> `check_access()` / `check_write_permission()` vérifient les permissions
> fines au niveau de chaque outil MCP. Les deux sont nécessaires.
...
- [ ] `client.py` — Client Streamable HTTP générique
...
| **Middleware sans 401**      | `/mcp` accessible sans token (erreur JSON-RPC au lieu de 401) | Bloc `if token_info is None: → 401` **obligatoire** (§5.3) |
| Auth oubliée dans un outil   | Un outil expose des données sans contrôle             | `check_access()` systématiquement        |
...
│       ├── client.py        # Client Streamable HTTP complet
...
│   └── Caddyfile            # Config WAF : route /mcp, /api/*, OWASP CRS
...
| Client Streamable HTTP | `scripts/cli/client.py`              |
